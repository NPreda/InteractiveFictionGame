...
public class pcDerivedStat : RPGStat
{
    ...
    private RPGStatType _connectedType;             //which type of attribute is connected with this stat
    private int _limitMin = 10;                           //the lower limit of the StatBaseMax variable (ex: your minimum health bar)
    private bool _isHidden = false;


    ///set what other stat/attribute it is connected to.
    public virtual RPGStatType ConnectedType{
        get{
            return _connectedType;
        }
        set{
            _connectedType = value;
        }
    }
    ...
    /// CONSTRUCTORS
    ///
    public pcDerivedStat() : base(){                          //basic constructor
        this.StatName = string.Empty;
        this.StatBaseMax = this.LimitMin; 
        this.StatBaseValue = this.StatBaseMax;    
        this.ConnectedType = 0;  
    }

    public pcDerivedStat(string name, RPGStatType statType)  : base() { //full constructor
        this.StatName = name;
        this.StatBaseMax = this.LimitMin; 
        this.StatBaseValue = this.StatBaseMax;
        this.ConnectedType = statType;
    }
}
